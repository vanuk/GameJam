...
public class FlyingEnemy : BaseEnemy
{
    ...
    {
        var origin = transform.position;
       if (Physics2D.Raycast(origin, Vector2.up, 0.7f, wallLayer) && CompareFloat(verticalDir, 1))
            verticalDir = -1;
        else if (Physics2D.Raycast(origin, Vector2.down, 0.7f, wallLayer) && CompareFloat(verticalDir, -1))
           verticalDir = 1;
    ...
    public void Idle()
    {
        transform.Translate(new Vector3(dir * movementSpeed, verticalDir * verticalSpeed, 0) * Time.deltaTime);
    }
